...
class one{
	...
	static function upload_file($localfile, $remotefile=null){
		...
		print ' 遠程資料：'.$remotepath.PHP_EOL;
		
		$filesize = onedrive::_filesize($localfile) OR die('無法獲取資料大小');
		if($filesize < 10485760){
			print ' 上傳方式：直接上傳'.PHP_EOL;
			$begin_time = microtime(true);
			$result = onedrive::upload($remotepath, file_get_contents($localfile));
			if(!empty($result)){
				$upload_time = microtime(true) - $begin_time;
				print ' 上傳成功:'.onedrive::human_filesize($filesize/$upload_time).'/s'.PHP_EOL;
			}else{
				print ' 上傳失敗!'.PHP_EOL;
			}
		}else{
			print ' 上傳方式：分塊上傳'.PHP_EOL;
	...
	static function upload_large_file($localfile, $remotepath){
		...
		if(empty($info['url'])){
			print ' 獲取會話失敗！'.PHP_EOL;
			sleep(3);
			return self::upload_large_file($localfile, $remotepath);
		}
		
		print ' 上傳分塊'.onedrive::human_filesize($info['length']).'	';
		$begin_time = microtime(true);
